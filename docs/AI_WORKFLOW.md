...
| P2A — Pairing backend/protocol | E/I (CLOSED) | Complete — Hermes H1 + Ackline A1 implemented + physically integrated (PASS_WITH_FINDINGS, docs/P2A_QA_RESULTS.md) |
| P2B — Guided onboarding + re-pair | D/F (CURRENT) | ChatGPT UX plan → Android/Compose implementation → Hermes small operator-facing QR/code tooling → security/privacy review around QR/token exposure → physical Oppo visual/interaction QA → screenshot review → ChatGPT GitHub |
